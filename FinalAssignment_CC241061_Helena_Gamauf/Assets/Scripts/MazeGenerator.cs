...
public class MazeGenerator : MonoBehaviour
{
    ...
    public MazeCell[,] GetMaze()
    {
        ...
        Random.InitState(MazeThemeManager.MazeLayoutSeed);
        ...
        int mazeStartX = Random.Range(0, mazeWidth);
        int mazeStartY = Random.Range(0, mazeHeight);
        CurvePath(mazeStartX, mazeStartY);
    ...
    List<Direction> directions = new List<Direction>()
{
    ...
    void CurvePath(int x, int y)
    {
        if (x < 0 || y < 0 || x > mazeWidth - 1 || y > mazeHeight - 1)
        {
            x = y = 0;
            Debug.LogWarning("Starting position out of bounds, so 0 instead");
        }
